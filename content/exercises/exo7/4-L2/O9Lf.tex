...
\contenu{
\texte{({\it 4 points}) On suppose qu'une population $x$ de lapins et une population $y$ de loups sont gouvern\'ees par le syst\`eme suivant d'\'equations diff\'erentielles :
$$(S)\ \ \left\{ \begin{align*}x'&= 4x-2y \\  y'&= x+y\end{align*}\right.$$
\begin {enumerate}}
\begin{enumerate}
    \item \question{Diagonaliser la matrice 
$$A=\begin{pmatrix}4&-2 \\ 1&1 \\ \end{pmatrix}.$$}
    \item \question{Exprimer le syst\`eme $(S)$ et ses solutions dans une base de vecteurs propres de $A$.}
    \item \question{Repr\'esenter graphiquement les trajectoires de $(S)$ dans le rep\`ere $(Oxy)$.}
    \item \question{Discuter graphiquement l'\'evolution de la population des lapins en fonction des conditions initiales.
\end {enumerate}}
\end{enumerate}
\reponse{
