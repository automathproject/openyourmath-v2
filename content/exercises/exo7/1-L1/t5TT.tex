\uuid{t5TT}
\exo7id{955}
\titre{exo7 955}
\auteur{cousquer}
\organisation{exo7}
\datecreate{2003-10-01}
\isIndication{false}
\isCorrection{false}
\chapitre{Application linéaire}
\sousChapitre{Image et noyau, théorème du rang}
\module{Algèbre}
\niveau{L1}
\difficulte{}

\contenu{
\texte{$E$ étant un espace vectoriel de dimension $n$ sur $\mathbb{R}$, $f$ une
application linéaire de~$E$ dans~$E$, construire dans
les trois cas suivants deux applications linéaires bijectives $u$ et $v$ de~$E$
dans~$E$ telles que $f=u-v$.}
\begin{enumerate}
    \item \question{$f$ est bijective.}
    \item \question{$\mathrm{Ker} f+\mathrm{Im} f=E$.}
    \item \question{$f$ est quelconque.}
\end{enumerate}
}
