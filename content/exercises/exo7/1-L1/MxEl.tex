\uuid{MxEl}
\exo7id{154}
\titre{exo7 154}
\auteur{bodin}
\organisation{exo7}
\datecreate{1998-09-01}
\isIndication{false}
\isCorrection{false}
\chapitre{Logique, ensemble, raisonnement}
\sousChapitre{Récurrence}
\module{Algèbre}
\niveau{L1}
\difficulte{}

\contenu{
\texte{\textbf{En quoi le raisonnement suivant est-il faux ?} \\
Soit $\mathcal{P}(n)$ : $n$ crayons de couleurs sont tous de la m\^eme couleur.}
\begin{enumerate}
    \item \question{$\mathcal{P}(1)$ est vraie car un crayon de couleur est de la m\^eme couleur
que lui-m\^eme.}
    \item \question{Supposons $\mathcal{P}(n)$. Soit $n+1$ crayons. On en retire $1$. Les $n$
crayons restants sont de la m\^eme couleur par hypoth\`ese de r\'ecurrence.

Reposons ce crayon et retirons-en un autre ; les $n$ nouveaux crayons sont \`a nouveau
de la m\^eme couleur. Le premier crayon retir\'e \'etait donc bien de la m\^eme couleur que les $n$ autres. La proposition est donc vraie au rang $n+1$.}
    \item \question{On a donc d\'emontr\'e que tous les crayons en nombre infini d\'enombrable
sont de la m\^eme couleur.}
\end{enumerate}
}
