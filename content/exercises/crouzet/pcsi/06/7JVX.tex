...
\reponse{$f$ est définie, continue et dérivable sur $\R$ comme quotient de fonctions dérivables ($\exp$) dont le dénominateur ne s'annule pas. On a :
...
\begin{center}
	\begin{tikzpicture}
		\begin{axis}[axis lines = middle, width = 11cm, height = 6cm,
			xmin = -6.5, xmax = 6.5, ymin = -0.2, ymax = 1.3,
			ytick = {0.5, 1}, xlabel = $x$, ylabel = $y$]
			\addplot[dashed, gray, domain = -6.5:6.5] {1} node[pos = 0.1, above] {$y = 1$};
			\addplot[thick, blue, domain = -6.5:6.5, samples = 200] {exp(x)/(exp(x) + 1)};
		\end{axis}
	\end{tikzpicture}
\end{center}
...
Par quotient, $\ds{\lim_{x\rightarrow 0^+} \frac{\ln(x)}{x} = -\infty}$. Par composée, \[ \lim_{x\rightarrow 0^+} g(x)=0 \]
...
et par composée :
 ...
 \begin{eqnarray*}
 	1-\ln(x) > 0 &\Leftrightarrow& 1> \ln(x) \\ &\Leftrightarrow& \mathrm{e} > x \quad \text{par stricte croissance de $\ln$}
 \end{eqnarray*}
 avec $\ds{g(\mathrm{e}) = \eu{\frac{\ln(\mathrm{e})}{\mathrm{e}}} = \mathrm{e}^\frac{1}{\mathrm{e}}}$.
Ainsi, on dispose du tableau de variation suivant :
\begin{center}
\begin{tikzpicture}
	 \tkzTabInit{$x$ / 1 , $g'(x)$ / 1, var de $g$ / 1.5}{$0$, $\mathrm{e}$, $+\infty$}
	 \tkzTabLine{d, +, z, -, }
	 \tkzTabVar{D-/ $0$, +/ $\eu{\frac{1}{\mathrm{e}}}$, -/ $1$}
\end{tikzpicture}
\end{center}
...
\begin{center}
	\begin{tikzpicture}
		\begin{axis}[axis lines = middle, width = 11cm, height = 6cm,
			xmin = 0, xmax = 15.5, ymin = 0, ymax = 1.7,
			xtick = {2.71828, 5, 10, 15}, xticklabels = {$\mathrm{e}$, $5$, $10$, $15$},
			ytick = {1, 1.44467}, yticklabels = {$1$, $\mathrm{e}^{1/\mathrm{e}}$},
			xlabel = $x$, ylabel = $y$]
			\addplot[dashed, gray, domain = 0:15.5] {1} node[pos = 0.9, below] {$y = 1$};
			\draw[dotted] (2.71828, 0) -- (2.71828, 1.44467) -- (0, 1.44467);
			\addplot[thick, blue, domain = 0.1:15.5, samples = 300] {exp(ln(x)/x)};
		\end{axis}
	\end{tikzpicture}
\end{center}
...
\begin{center}
	\begin{tikzpicture}
		\begin{axis}[axis lines = middle, width = 9cm, height = 7cm,
			xmin = -3.2, xmax = 3.2, ymin = 0, ymax = 10.5,
			ytick = {1, 5, 10}, xlabel = $x$, ylabel = $y$]
			\addplot[thick, blue, domain = -3:3, samples = 200] {(exp(x) + exp(-x))/2};
		\end{axis}
	\end{tikzpicture}
\end{center}
...
On détermine le signe de $x^4-7x^2+6$ par la méthode classique. On pose $X=x^2$. Les solutions de $X^2-7X+6$ sont $6$ et $1$. On peut alors écrire
...
\begin{center}
	...
	\end{tikzpicture}
\end{center}
et la courbe représentative suivante, avec ses asymptotes :
\begin{center}
	\begin{tikzpicture}
		\begin{axis}[axis lines = middle, width = 11cm, height = 9cm,
			xmin = -5, xmax = 5, ymin = -6, ymax = 6,
			xtick = {-4, 4}, ytick = {-4, -2, 2, 4},
			extra x ticks = {-1.73205, 1.73205},
			extra x tick labels = {$-\sqrt{3}$, $\sqrt{3}$},
			extra x tick style = {tick label style = {below right}},
			xlabel = $x$, ylabel = $y$]
			\draw[dashed, gray] (-1.73205, -6) -- (-1.73205, 6);
			\draw[dashed, gray] (1.73205, -6) -- (1.73205, 6);
			\addplot[dashed, gray, domain = -5:5] {(x + 1)/2} node[pos = 0.8, below right] {$y = \frac{x+1}{2}$};
			\addplot[thick, blue, domain = -5:-1.74, samples = 200] {(x^3 + x^2 - 2*x - 3)/(2*x^2 - 6)};
			\addplot[thick, blue, domain = -1.725:1.725, samples = 200] {(x^3 + x^2 - 2*x - 3)/(2*x^2 - 6)};
			\addplot[thick, blue, domain = 1.74:5, samples = 200] {(x^3 + x^2 - 2*x - 3)/(2*x^2 - 6)};
		\end{axis}
	\end{tikzpicture}
\end{center}}
}
