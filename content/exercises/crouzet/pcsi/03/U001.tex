...
\reponse{\begin{align*}
	...
	&= \sum_{j=1}^n \frac{j^2}{2}+\frac{j}{2} \\
...
\reponse{\begin{align*}
	\sum_{1\leq i,j\leq n} (i+j) &= \sum_{j=1}^n \sum_{i=1}^n (i+j) \\
	&= \sum_{j=1}^n \left(\sum_{i=1}^n i + \sum_{i=1}^n j\right)\\
	&= \sum_{j=1}^n \frac{n(n+1)}{2}+nj = n\frac{n(n+1)}{2}+ n\frac{n(n+1)}{2}=n^2(n+1)
\end{align*}}
\question{$\ds{\sum_{1\leq i<j\leq n} (i+j)}$}
\reponse{\begin{align*}
	\sum_{1\leq i<j\leq n} (i+j) &= \sum_{j=2}^n \sum_{i=1}^{j-1} (i+j)  &\text{par Fubini}\\
...
\reponse{On constate que $(i,j)\in A_n$ si et seulement si $i\in \interent{0 n}$ et $j=n-i$. On peut alors écrire
\begin{align*}
